...
\begin{document}
  ...
  \node(subject) [object] {};
  \node(subjectName) [below] at (subject.north) {\bf Subject1};
  \node(event1) [token] at (subject.east) {};

  \node(subject2) [object,below=of subject,xshift=5mm] {};
  \node(subjectName2) [below] at (subject2.north) {\bf Subject2};
  \node(event2) [token] at (subject2.east) {};
  ...
  \node(observer3) [right=5cm of subject,yshift=-7cm,object] {};
  ...
  \coordinate(a1) at ($(event2.east)+(25mm,0)$);
  ...
  \draw[->,dashed,-{Stealth}] (event2) .. controls (a1) and (c) .. (method2);
  \draw[->,dashed,-{Stealth}] (event2) .. controls (a1) and (d) .. (method3);

  \node [left] at (event1.west) {\ttfamily event1};
  \node [left] at (event2.west) {\ttfamily event2};
