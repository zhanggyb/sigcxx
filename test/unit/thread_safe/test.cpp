...
void thread1 () {
  ...
  for(int i = 0; i < 100; i++) {
    ...
    lock.lock();
    s.event1().Connect(&c, static_cast<void (Consumer::*)(const Sender*, int)>(&Consumer::OnTest1));
    lock.unlock();

    s.DoTest1(1);

    lock.lock();
    s.event1().DisconnectAll(&c, static_cast<void (Consumer::*)(const Sender*, int)>(&Consumer::OnTest1));
    lock.unlock();
...
void thread3 () {
  ...
  for(int i = 0; i < 500; i++) {
    ...
    lock.lock();
    s.event1().Connect(lc, static_cast<void (Consumer::*)(const Sender*, int)>(&Consumer::OnTest1));
    lock.unlock();
...
void thread4 () {
  ...
  for(int i = 0; i < 500; i++) {
    ...
    lock.lock();
    ls->event1().Connect(&c, static_cast<void (Consumer::*)(const Sender*, int)>(&Consumer::OnTest1));
    lock.unlock();
